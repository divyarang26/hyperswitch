...
pub trait ConnectorActions: Connector {
    ...
    async fn authorize_and_capture_payment(
        ...
        let authorize_response = match self
            .authorize_payment(authorize_data, payment_info.clone())
            .await
        {
            Ok(res) => {
                if res.status != enums::AttemptStatus::Authorized {
                    return Err(Report::from(ConnectorError::MissingRequiredField {
        ...
        let txn_id = get_connector_transaction_id(authorize_response.response)
            .ok_or_else(|| Report::from(ConnectorError::MissingRequiredField {
                field_name: "Transaction ID",
            }))?;
        let response = match self
            .capture_payment(txn_id, capture_data, payment_info)
            .await
        {
            Ok(res) => res,
            Err(_) => {
